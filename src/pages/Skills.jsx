...
import {
  ...
  FaFlask, FaBug, FaGitAlt, FaGithub, FaNpm,
  FaChrome, FaUserFriends, FaLightbulb, FaClock, FaSyncAlt,
  FaMobileAlt, FaExchangeAlt, FaServer, FaDatabase, FaCogs, FaShareAlt,
  FaHdd, FaRocket, FaCloud, FaCubes, FaTachometerAlt, FaUniversalAccess, FaCheckCircle,
  FaCode
} from 'react-icons/fa';
...
const calculateExperience = (startYear) => {
  ...
  return years === 1 ? `${years} Year of Experience` : `${years} Years of Experience`;
};
...
function Skills() {
  ...
  return (
    <div className="skills-wrapper position-relative">
      ...
      />
      <Animated.div style={fadeIn} className="container-fluid skills-wrapper mt-0">
        <Container>
          <h1 className="skills-heading">Frontend Force</h1>
          <AnimatedHR />
          {skillCategories.map((section, idx) => (
            <div key={idx} className="mb-0">
              <div className="section-subheading w-100 text-start">{section.title}</div>
              <Row className="g-4 mt-3">
                {section.skills.map((skill, i) => (
                  <Col xs={12} md={12} lg={12} key={i}>
                    <Animated.div style={cardAnimation}>
                      <Card className="modern-skill-card" style={{ borderRadius: 0, border: '2px solid #4c3b6e' , background: 'none' }}>
